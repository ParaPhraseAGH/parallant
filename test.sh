    ...
    for model in $models; do
    	for agents_impl in $agents_impls; do
        	...
        	erl -pa ebin -pa deps/*/ebin \
            	-eval "parallant:start($Width,$Height,$Agents,$Steps,[{algorithm,$algorithm},{model,$model},{agents_impl,$agents_impl},{log,false},{tiles_per_colour,$tiles_per_colour},{workers_per_colour,$workers_per_colour}])." \
