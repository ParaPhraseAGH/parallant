    ...
    for model in $models; do
    	for agents_impl in $agents_impls; do
        	...
        	erl -pa ebin -pa deps/*/ebin \
            	-eval "parallant:start($Width,$Height,$Depth,$Agents,$Steps,[{algorithm,$algorithm},{model,$model},{agents,$agents_impl},{custom_log_interval,10},{log_world,false},{tiles_per_colour,$tiles_per_colour},{workers_per_colour,$workers_per_colour}])." \
